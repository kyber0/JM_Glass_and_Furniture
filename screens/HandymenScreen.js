...
import { handymenAPI, shopAPI } from '../services/api';
...
const HandymenScreen = ({ navigation }) => {
    const { user } = useAuth();
    ...
    const loadHandymen = useCallback(async () => {
        ...
        try {
            ...
            setShopId(sid);
            const res = await handymenAPI.getByShop(sid);
            if (res.success) setHandymen(res.handymen);
        } catch (e) {
            console.error('Load handymen error:', e);
        } finally {
            setLoading(false);
        }
    }, [user]);
    ...
    // ── Stats ─────────────────────────────────────────────────────────────────
    const stats = [
        { label: 'Total',     value: handymen.length,                                color: theme.accent, icon: 'people',                bg: theme.accentBg || '#efebe9' },
        { label: 'Available', value: handymen.filter(h => h.status === 'available').length, color: '#2e7d32', icon: 'checkmark-circle', bg: '#e8f5e9' },
        { label: 'Busy',      value: handymen.filter(h => h.status === 'busy').length,      color: '#e65100', icon: 'time',              bg: '#fff3e0' },
        { label: 'Off Duty',  value: handymen.filter(h => h.status === 'off').length,       color: '#546e7a', icon: 'moon',              bg: '#eceff1' },
    ];
    ...
    const openWorkload = async (item) => {
        ...
        try {
            const res = await handymenAPI.getHandymanOrders(item.handyman_id);
    ...
    const openAdd = () => {
        setEditing(null);
        setForm({ name: '', phone: '', status: 'available' });
        setModalVisible(true);
    };

    const openEdit = (item) => {
        setEditing(item);
        setForm({ name: item.name, phone: item.phone || '', status: item.status });
        setModalVisible(true);
    };

    const handleSave = async () => {
        if (!form.name.trim()) {
            showAlert('Required', "Please enter the handyman's name.", 'warning');
            return;
        }
        setSaving(true);
        try {
            if (editing) {
                await handymenAPI.update(editing.handyman_id, {
                    name: form.name, phone: form.phone, status: form.status,
                });
            } else {
                await handymenAPI.add(shopId, form.name, form.phone);
            }
    ...
    const handleDelete = (item) => {
        showAlert(
            'Remove Handyman',
            `Remove ${item.name} from your team? All their order assignments will also be cleared.`,
            'warning', true,
            async () => { await handymenAPI.remove(item.handyman_id); loadHandymen(); }
        );
    };
    ...
    const handleCycleStatus = async (item) => {
        ...
        await handymenAPI.update(item.handyman_id, { status: next });
        setHandymen(prev => prev.map(h => h.handyman_id === item.handyman_id ? { ...h, status: next } : h));
    };

    // ── Render Card ───────────────────────────────────────────────────────────
    const renderItem = ({ item }) => {
        const cfg        = STATUS_CONFIG[item.status] || STATUS_CONFIG.available;
        const ac         = avatarColor(item.handyman_id);
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top']}>
            ...
            <View style={[styles.header, { borderBottomColor: theme.border, backgroundColor: theme.headerBg }]}>
                ...
                <View>
                    <Text style={[styles.headerTitle, { color: theme.headerText }]}>Installation Team</Text>
                    {!loading && (
                        <Text style={[styles.headerSub, { color: theme.accent }]}>
                            {handymen.length} member{handymen.length !== 1 ? 's' : ''}
                        </Text>
                    )}
                </View>
                    ...
                    ListEmptyComponent={
                        <View style={styles.empty}>
                            ...
                            <Text style={[styles.emptySub, { color: theme.textMuted }]}>
                                Add your installation crew so customers can book installation services.
                            </Text>
                            <TouchableOpacity style={[styles.emptyBtn, { backgroundColor: theme.accent }]} onPress={openAdd}>
                                <Ionicons name="person-add-outline" size={18} color="#fff" />
                                <Text style={styles.emptyBtnText}>Add First Handyman</Text>
                            </TouchableOpacity>
                        </View>
                    }
                />
                ...
                <View style={styles.overlay}>
                    ...
                    <View style={[styles.modal, { backgroundColor: theme.card }]}>
                        ...
                        <View style={styles.modalHeaderRow}>
                            <View style={[styles.modalAvatar, { backgroundColor: form.name ? avatarColor(form.name.charCodeAt(0)) : theme.inputBg }]}>
                                ...
                                >
                                    {saving
                                        ...
                                        : <>
                                            ...
                                            <Text style={styles.saveText}>{editing ? 'Save Changes' : 'Add Handyman'}</Text>
                                          </>
                                    }
                                </TouchableOpacity>
                            </View>
            ...
            <Modal visible={!!workloadSheet} transparent animationType="slide" onRequestClose={() => setWorkloadSheet(null)}>
                <View style={styles.overlay}>
                    ...
                    <View style={[styles.workloadSheet, { backgroundColor: theme.card }]}>
                        ...
                        {workloadSheet && (() => {
                            ...
                            return (
                                <View style={[styles.workloadHeader, { backgroundColor: cfg.bg, borderRadius: 16, marginBottom: 20 }]}>
                                    <View style={[styles.workloadAvatar, { backgroundColor: avatarColor(workloadSheet.handyman_id) }]}>
                                        <Text style={styles.workloadAvatarText}>{initials(workloadSheet.name)}</Text>
                                    </View>
                                    <View style={{ flex: 1 }}>
                                        <Text style={[styles.workloadName, { color: theme.text }]}>{workloadSheet.name}</Text>
                        ...
                        <Text style={[styles.sheetSectionTitle, { color: theme.textMuted }]}>Active Assignments</Text>
                                ...
                                {workloadOrders.map(order => (
                                    <View key={order.order_id} style={[styles.orderRow, { borderColor: theme.border, backgroundColor: theme.background }]}>
                                        ...
                                        <View style={{ alignItems: 'flex-end' }}>
                                            <View style={[styles.orderStatusPill, { backgroundColor: STATUS_CONFIG.busy.bg }]}>
                                                <Text style={[styles.orderStatusText, { color: STATUS_CONFIG.busy.color }]}>{order.status}</Text>
                                            </View>
