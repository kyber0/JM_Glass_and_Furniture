...
const AdminDeliveryMenScreen = ({ navigation }) => {
    ...
    const fetchDeliveryMen = useCallback(async () => {
        try {
            ...
            if (res.success) { setDeliveryMen(res.data); applyFilter(res.data, search); }
        } catch (e) { console.error(e); }
        finally { setLoading(false); setRefreshing(false); }
    }, []);

    useFocusEffect(useCallback(() => { setLoading(true); fetchDeliveryMen(); }, [fetchDeliveryMen]));

    const applyFilter = (data, q) => {
        if (!q.trim()) { setFiltered(data); return; }
        setFiltered(data.filter(d =>
            d.name.toLowerCase().includes(q.toLowerCase()) ||
            d.shop_name?.toLowerCase().includes(q.toLowerCase()) ||
            d.vehicle_type?.toLowerCase().includes(q.toLowerCase()) ||
            d.plate_number?.toLowerCase().includes(q.toLowerCase())
        ));
    };

    const handleSearch = (q) => { setSearch(q); applyFilter(deliveryMen, q); };

    const renderDeliveryMan = ({ item }) => (
        <View style={[styles.card, { borderBottomColor: theme.border }]}>
            <View style={[styles.avatar, { backgroundColor: '#1565C0' + '22' }]}>
                <Ionicons name="car" size={22} color="#1565C0" />
            </View>
            <View style={styles.info}>
                <Text style={[styles.name, { color: theme.text }]}>{item.name}</Text>
                <View style={styles.metaRow}>
                    <Text style={[styles.spec, { color: theme.textMuted }]}>{item.vehicle_type || 'Unknown Vehicle'}</Text>
                    {item.plate_number && <>
                        <Text style={[styles.dot, { color: theme.textMuted }]}>·</Text>
                        <Text style={[styles.spec, { color: theme.textMuted }]}>{item.plate_number}</Text>
                    </>}
                </View>
                <View style={styles.metaRow}>
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top']}>
            <View style={[styles.header, { backgroundColor: theme.headerBg, borderBottomColor: theme.border }]}>
                <Ionicons name="car" size={20} color="#1565C0" style={{ marginRight: 10 }} />
                <Text style={[styles.headerTitle, { color: theme.headerText }]}>Delivery Men</Text>
                <View style={[styles.countBadge, { backgroundColor: '#1565C0' + '22' }]}>
                    <Text style={[styles.countText, { color: '#1565C0' }]}>{filtered.length}</Text>
                </View>
            </View>
            ...
            <View style={[styles.searchBox, { backgroundColor: theme.inputBg, borderColor: theme.border }]}>
                ...
                <TextInput
                    ...
                    placeholder="Search name, shop, vehicle, plate..."
            ...
            {loading ? <ActivityIndicator size="large" color="#1565C0" style={{ marginTop: 40 }} /> : (
                <FlatList
                    data={filtered}
                    renderItem={renderDeliveryMan}
                    keyExtractor={item => item.delivery_man_id.toString()}
                    contentContainerStyle={styles.list}
                    refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchDeliveryMen(); }} tintColor="#1565C0" />}
                    ListEmptyComponent={
                        <View style={styles.emptyBox}>
                            <Ionicons name="car-outline" size={52} color={theme.textMuted} />
                            <Text style={[styles.emptyText, { color: theme.textMuted }]}>No delivery men registered</Text>
                        </View>
                    }
                />
            )}
...
export default AdminDeliveryMenScreen;
